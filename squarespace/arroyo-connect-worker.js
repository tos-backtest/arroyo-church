/**
 * Arroyo Church — "Join a Group" relay (Cloudflare Worker).
 *
 * Receives the on-site form POST from arroyochurch.com, validates + anti-spam,
 * and creates a Planning Center People form submission (form 1206123) using a
 ...
 *   "What's your next step?"  9567926 -> value = option id (we always send "join a group")
 *        10625856 placed faith · 10558271 Connect Class · 10558272 join a group
 *        10648022 baptized · 10558273 start serving
 *   "Join a Connect Group"   9567985 -> value = option id (one entry per group)
 ...
 *   "Which team"             9568011 -> value = option id (unused here)
 *   Message (text)           9630046 -> value = text
...
const F_PHONE = "9974695", F_NEXT = "9567926", F_GROUP = "9567985", F_MESSAGE = "9630046";
const NEXT_STEP_JOIN = "10558272"; // "I want to join a Connect Group"
// Whitelist of valid Connect Group option ids — anything else is ignored.
...
export default {
  async fetch(request, env) {
    ...
    if (!first || !last || !email || !phone) return json({ success: false, error: "missing" }, 422, origin);
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) return json({ success: false, error: "email" }, 422, origin);

    const values = [
      { form_field_id: F_PHONE, value: phone },
      { form_field_id: F_NEXT, value: NEXT_STEP_JOIN },
    ];
    if (b.group && GROUP_OPTIONS.has(String(b.group))) {
      values.push({ form_field_id: F_GROUP, value: String(b.group) });
    }
    if (b.message && String(b.message).trim()) {
      values.push({ form_field_id: F_MESSAGE, value: String(b.message).trim().slice(0, 2000) });
    }
